...
switch($_SESSION["step"]) {
    case 0:
        if($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            // Generate SMS token
            file_get_contents($token_ws."?action=generate_sms&token=1111&msisdn=".urlencode($_POST['phone']));
    ...
    case 1:
        if($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if(isset($_COOKIE['CockpitCaptchaText']) && isset($_COOKIE['CockpitMsisdnKey']) && isset($_COOKIE['CockpitSmsTokenKey'])) {
                ...
                $raw = base64_decode($_COOKIE['CockpitSmsTokenKey']);
                $iv = substr($raw, 0, $iv_length);
                $cipher = substr($raw, $iv_length);
                $smstoken = openssl_decrypt($cipher, $method, $key, OPENSSL_RAW_DATA, $iv);
